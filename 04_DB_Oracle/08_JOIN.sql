/*
    ...
    - 관계형 데이터 베이스는 최소한의 데이터로 각각의 테이블에 담고 있음(중복을 최소화하기 위해 최대한 쪼개서 관리함)
...
/*
    ...
    - 연결 시키는 컬럼의 값이 일치하는 행들만 조인되서 조회
...
*/
-- EMPLOYEE, DEPARTMENT 조인해서 사번(EMP_ID), 사원명(EMP_NAME), 부서코드(DEPT_CODE), 부서명(DEPT_TITLE) 조회
