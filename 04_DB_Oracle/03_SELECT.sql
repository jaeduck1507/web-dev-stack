/*
    ...
    - 관계형(Relational) 데이터베이스
        ...
        3. 데이터의 중복을 최소화하고 무결성, 트랜잭션 관리 등 데이터 관리 성능을 제공
    ...
    - SQL 종류
        1. DML(Data Manipulation Language) : 데이터 조작어
            ...
            - SELECT는 데이터 조회를 의미하여 일부에서는 DQL(Data query Language)
        2. DDL(Data Defintion Language) : 데이터 정의어
...
/*
    ...
    - 조회하고자 하는 컬럼은 반드시 from 절에 지정한 테이블에 존재해야한다.
...
/*
    컬럼 산술 연산
    - SELECT 절에서 컬럼명 입력 부분에 산술연산자를 사용하여 결과 조회
    
    컬럼 별칭
    - 산술연산을 하게 되면 컬럼명이 지저분해진다.
    이때 컬럼명에 별칭을 부여하면 깔끔하게 보일 수 있다.
    컬럼 AS 별칭 / 컬럼 AS "별칭" / 컬럼 "별칭"
*/
-- USER_INFO에서 나이(AHE)에 10년 후를 계산
SELECT 
    NAME,
    AGE,
    AGE+10 AS "10년 후"
...
SELECT
NAME || '의 MBTI는 ' || MBTI || '입니다' AS "MBTI 소개"
FROM USER_INFO;

/*
      DISTINCT 
        - 컬럼에 중복된 값들을 한번씩만 표시하고자 할때
        - 주의사항 : DISTINCTSMS SELECT절에서 한 번만 기술 가능 
*/
-- USER_INFO에서 나이(AGE) 중복되지 않게 조회
SELECT DISTINCT AGE FROM USER_INFO;

-- USER_INFO에서 MBTI 중복되지 않게
...
-- 직급코드(JOB_CODE)가 J7이거나 J2인 사원들 중 
--                              급여가 200만원인 사원들 조회
-- IN 연산자 사용 / OR 연산자 
SELECT * FROM EMPLOYEE
...
-- 평균 급여 SALARY를 조회 (부서코드 DEPT_CODE) 
-- DEPT_CODE가 NULL이 아닌 경우
SELECT
    DEPT_CODE,
    TO_CHAR(ROUND(AVG(NVL(SALARY,0))),'9,999,999')
FROM EMPLOYEE
GROUP BY DEPT_CODE
...
-- USER_INFO에서 MBTI별 평균나이를 계산하는데 평균나이가 30이하인 MBTI만 조회
