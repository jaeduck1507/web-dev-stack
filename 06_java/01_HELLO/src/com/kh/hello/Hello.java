/*
 * * 자바의 특징
 * 1. 객체지향 프로그래밍(Object_Oriented Programming, OPP) 언어
 * 		- 모든 기능을 "객체"라는 단위로 구성
 * 		- 캡슐화, 상속, 다형성, 추상화
 ...
 * 		- 메모리 효율
 * 4. 운영체제에 독립적 (JVM)
 * 		- 자바코드는 JVM 위에서 돌아가기 때문에 window, macOs, Linux 모두 실행 가능
...
public class Hello { // 클래스명은 클래스 파일명과 동일해야 한다.
	 ...
	 * - 해당 메스드 호출 시 메서드의 블럭({}) 에 해당하는 부분을 실행한다.
	 * */
	...
	public static void main(String[] args) {
		...
		/*
		 * 네이밍 규칙
		 * - 패키지명 : 소문자로 작성, 회사들 간에 패키지가 중복되지 않도록 회사의 도메인 이름으로 패키지를 만든다.
		 * - 클래스명 : 대문자로 시작, 여러개의 단어가 있을때에는 단어 앞자리마다 대문자
		 * - 메서드명 : 소문자로 시작, 여러개의 단어가 있을때에는 단어 앞자리마다 대문자
		 * */
